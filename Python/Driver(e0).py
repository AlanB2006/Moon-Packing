...
NPROC     = int(sys.argv[4]) if len(sys.argv)>4 else mp.cpu_count()

# embed setup in filename so multiple runs don't overwrite each other
TFLAG   = "T1" if TIDES_ON else "T0"
...
def run_simulation(param):
    ...
    am  = np.array([A_M0]+[get_semi(dH,A_M0,i,0) for i in range(1,N_MOONS)])
    ...
    for i in range(N_MOONS):
        sim.add(m=M_M,a=am[i],e=e0,r=R_M,f=get_true_anomaly(i),primary=sim.particles["Earth"],hash="M%d"%(i+1))
    sim.move_to_com()
    if TIDES_ON: setup_tides(sim)
    ps=sim.particles

    P_M=np.sqrt(am[0]**3/M_E)          # period at actual inner moon (2*R_roche)
    tmax=1e5*P_M                       # total run length in inner-moon orbits
...
# parameter grid
par_dH=np.arange(3.5,13.5,0.01)
par_e =np.arange(0.0,0.301,0.01)
PARAMS=[(d,e0) for e0 in par_e for d in par_dH]
N_TOTAL=len(PARAMS)
