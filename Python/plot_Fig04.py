...
for col, i in enumerate(col):
    ...
    if i == 5:
        ...
        ax.semilogx(reb_data[1:,0], reb_spin, 'r.', ms=s)
        ax.semilogx(sec_data[1:,0], sec_spin, 'b.', ms=s)
        ax.set_ylabel("$\Omega_{\mathrm {spin}}\;(\mathrm{day^{-1}})$")
        ax.set_xlabel("Time (yr)")
        ax.tick_params(labelbottom=True)
    else:
        ax.semilogx(reb_data[1:,0], reb_data[1:,i], 'r.', ms=s)
        ax.semilogx(sec_data[1:,0], sec_data[1:,i], 'b.', ms=s)
