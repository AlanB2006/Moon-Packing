...
N_MOONS   = 2        # Number of moons (Integer)
MOON_TYPE = "L"       # "L" (Luna), "C" (Ceres), or "P" (Pluto)
TIDES_ON  = True      # True or False
TAU       = 100       # Tidal time lag (Used only if TIDES_ON is True)
NPROC     = mp.cpu_count() # Number of CPU cores to use
...
def run_simulation(delta_H):
    ...
    t = 0.0
    fate_label = "stable"
    # sim.save_to_file("L_3.bin", interval=P_M)
    ...
    try:
        ...
        if fate_label == "stable" and t >= tmax:
            fate_label = "tmax"
    except rebound.Collision:
        fate_label = "collision"

    # final semimajor axes and delta a / a0
    af = np.zeros(N_MOONS)
    for i in range(N_MOONS):
        af[i],_,_ = cart2orb(ps["Earth"],ps[2+i],M_E+M_M)
    delta_a_over_a0 = (af - am0) / am0
    wall = time.time() - t0
    out = "%1.3f, %1.2f, " % (delta_H, e0)
    for i in range(N_MOONS):
        out += "%1.4f, %1.4f, %1.4e, " % (emax[i], emin[i], delta_a_over_a0[i])
...
if __name__ == '__main__':
    hdr = "# dH, e0, "
    for i in range(N_MOONS):
        hdr += "eM%d_max, eM%d_min, dA%d_over_A0, " % (i+1, i+1, i+1)
    hdr += "t_end(PM), wall_time, fate\n"
    write_to_file(OUTFILE, hdr, mode="w")

    # delta_H
    par_dH = np.arange(7.2, 7.4, 0.001) # Luna: 9, Pluto: 7.5, Ceres: 14.0
    # par_dH = np.array([8.55])
    PARAMS = par_dH
    N_TOTAL = len(PARAMS)
    ...
    with mp.Pool(processes=NPROC) as pool:
        ...
        for _ in pool.imap_unordered(run_simulation, PARAMS):
            ...
            # The carriage return '\r' updates the line in place in Colab
