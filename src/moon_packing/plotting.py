...
PAPER_CONFIG = {
    "Luna": {"figure": 2, "sweep": "2_Luna_{tau}s_final.txt", "map": "output_L_2moons_{flag}_9.0_{tau}.txt", "xmax": 9.0},
    "Pluto": {"figure": 3, "sweep": "3_Pluto_{tau}s_final.txt", "map": "output_P_3moons_{flag}_11.0_{tau}.txt", "xmax": 11.0},
    "Ceres": {"figure": 4, "sweep": "5_Ceres_{tau}s_final.txt", "map": "output_C_5moons_{flag}_14.0_{tau}.txt", "xmax": 14.0},
}
...
def plot_lifetime_eccentricity(moon_type: str, root: str | Path | None = None,
    ...
    for col, tau in enumerate(TAUS):
        ...
        for i in range(1, table.n_moons + 1):
            axes[1, col].scatter(beta, table[f"eM{i}_max"], s=2,
                                 color=MOON_COLORS[i-1], rasterized=True)
        ...
        for row in range(2):
            ...
            ax.text(0.04, 0.88, panel[row*3+col], transform=ax.transAxes,
    ...
    handles = [plt.Line2D([], [], marker="s", ls="", color=MOON_COLORS[i], label=str(i+1))
               for i in range(model.manuscript_count)]
    fig.legend(handles=handles, title="Moon index", ncol=model.manuscript_count,
               loc="upper left", bbox_to_anchor=(0.08, 1.01), frameon=False)
...
def plot_eccentricity_map(moon_type: str, root: str | Path | None = None,
                          output: str | Path | None = None) -> Path:
    root = Path(root) if root else _repo_root()
    model = get_moon_model(moon_type)
    cfg = PAPER_CONFIG[model.name]
    ...
    for row, tau in enumerate(TAUS):
        ...
        for col, moon_index in enumerate(moon_indices):
            ...
            ax.text(0.03, 0.89, panel[row*2+col], transform=ax.transAxes,
                    fontsize=13, fontweight="bold")
            ax.axvline(beta_max(model.manuscript_count, model), color="black", ls="--", lw=1)
            # Draw selected low-order resonances without obscuring the heat map.
            for ratio in [3/2, 5/3, 2, 5/2, 3, 4, 5, 6, 7, 8, 9]:
                location = resonance_beta(ratio, model)
                if 3.5 < location < cfg["xmax"]:
                    ax.axvline(location, color="black", ls=":", lw=0.55, alpha=0.35)
            if row == 0:
                ax.set_title(f"Moon {moon_index}")
            if row == 2:
            ...
            ax.minorticks_on()
            ax.tick_params(which="both", direction="out")
        axes[row, 1].text(0.97, 0.92, rf"$\tau={tau}\,$s", transform=axes[row, 1].transAxes,
                          ha="right", va="top", fontsize=10,
                          bbox={"facecolor": "white", "alpha": 0.75, "edgecolor": "none", "pad": 1.5})
    if image is not None:
        cbar = fig.colorbar(image, ax=axes.ravel().tolist(), pad=0.02, fraction=0.035)
        cbar.set_label(r"$\log_{10}\Delta e$")
    fig.subplots_adjust(wspace=0.08, hspace=0.08, right=0.88)
...
def plot_time_series(moon_type: str, beta_values: list[float], root: str | Path | None = None,
                     output: str | Path | None = None) -> Path:
    root = Path(root) if root else _repo_root()
    model = get_moon_model(moon_type)
    paths = [root / "data" / "generated" / "timeseries" /
             f"{model.name.lower()}_beta{beta:g}_tau698.npz" for beta in beta_values]
    ...
    for col, (path, beta) in enumerate(zip(paths, beta_values)):
        ...
        for i in range(model.manuscript_count):
            ...
            if i > 0:
                axes[3, col].scatter(t, periods[:, i]/periods[:, i-1], s=0.4,
                                     color=MOON_COLORS[i], rasterized=True)
        axes[0, col].set_aspect("equal", adjustable="box")
        axes[0, col].set_xlim(-0.44, 0.44); axes[0, col].set_ylim(-0.44, 0.44)
        axes[0, col].set_title(rf"$\beta={beta:g}$")
        axes[2, col].set_yscale("log")
        axes[3, col].set_xlabel("Time (yr)")
        for row in range(4):
            axes[row, col].minorticks_on(); axes[row, col].tick_params(which="both", direction="out")
    labels = [r"$Y/R_H$", r"$a/R_H$", r"$e$", r"$P_{i+1}/P_i$"]
    for row, label in enumerate(labels): axes[row, 0].set_ylabel(label)
    for col in range(3): axes[0, col].set_xlabel(r"$X/R_H$")
...
def reproduce_figures(figures: list[int], root: str | Path | None = None,
